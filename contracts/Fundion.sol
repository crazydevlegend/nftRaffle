...
contract Funding is MyNFT{
    ...
    function buyNFT(uint256 _tokenID, address to) private
    {
        ...
        require(nftPrice == msg.value, "price is not equal!");
        // pay(msg.sender,nftOwner,nftPrice); //msg.sender가 owner에게 돈을 지불 Direct로 못준다
        console.log('_tokenID', _tokenID);
        //작품은 이걸 실행한 사람한테 간다
        _transfer(nftOwner,to,tokenID);   //token의 owner가
        // msg sender에게 토큰을 준다
    ...
    //어떤 nft에 펀딩하냐, 펀딩 금액 고정, contrac가 함수 실행sender의 0.1ether를 받아온다,이 함수가 실행되면 그냥 받아지는것
    ...
    function randomNFTOwner(uint256 _tokenID) public onlyOwner
    {
       uint256 ownerIndex = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)))%participants[_tokenID].length;

       address ownerAddress= participants[_tokenID][ownerIndex];
        buyNFT(_tokenID,ownerAddress); //NFT를 구매하게 할것이다 
        //초기화 시키는 로직
       //participants[_tokenID].clear();
